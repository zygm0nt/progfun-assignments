...
//    forall_logical(s, p, (x, y) => x && y, true)

/*  def forall_logical(s: Set, p: Int => Boolean, 
		  			l: (Boolean, Boolean) => Boolean,
		  			end: Boolean): Boolean = {*/
   ...
   * that satisfies `p`.
   */
  //def exists(s: Set, p: Int => Boolean): Boolean = forall_logical(s, p, (x, y) => x || y, false)
