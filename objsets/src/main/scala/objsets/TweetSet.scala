...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  /*def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = {
    if (tail.isEmpty) accu
    if (p(head)) tail.filter0(p, accu incl elem)
  ...
  override def union(that: TweetSet): TweetSet = {
    if (tail.isEmpty) that incl elem
    else tail.union(that incl elem)
  }
  ...
  def ascendingByRetweet: Trending = {
      def tweet = findMin
      new NonEmptyTrending(tweet, this.remove(tweet).ascendingByRetweet)
    }
}
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = {
    TweetReader.allTweets.filter(tw => google.exists(s => tw.text contains s))
  }

  val appleTweets: TweetSet = {
    TweetReader.allTweets.filter(tw => apple.exists(s => tw.text contains s))
  }
